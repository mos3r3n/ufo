...
#include "ufo/predictors/BrightnessTemperatureClrCldDiff.h"
...
static PredictorMaker<BrightnessTemperatureClrCldDiff>
       makerFuncBrightnessTemperatureClrCldDiff_("brightnessTemperatureClrCldDiff");

// -----------------------------------------------------------------------------

BrightnessTemperatureClrCldDiff::BrightnessTemperatureClrCldDiff(const Parameters_ & parameters,
...
void BrightnessTemperatureClrCldDiff::compute(const ioda::ObsSpace & odb,
                                            const GeoVaLs &,
  ...
  for (std::size_t jloc = 0; jloc < nlocs; ++jloc) {
    for (std::size_t jvar = 0; jvar < nvars; ++jvar) {
      ...
      if (clr[jloc] != fmiss && bak[jloc] != fmiss && clr[jloc] != bak[jloc]) {
        out[jloc*nvars + jvar] = clr[jloc] - bak[jloc];
      } else {
