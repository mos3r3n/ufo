...
subroutine ufo_directZDA_simobs(self, geovals, obss, nvars, nlocs, hofx)
  ...
  use ufo_constants_mod, only : t2tv, rd, one, pi, zero, ten, T_melt, &
                                rhor, rhoh, rhos, rhog, am_s
  ...
  real(kind_real) :: qx_min
  real(kind_real) :: qn_min
  real(kind_real) :: qb_min
...
! for single moment scheme (LIN)
  ...
  do ivar = 1,1
    do iobs=1,nlocs 
      ...
      qrges = max(vfields(1,iobs), qx_min)  ! qr
      qsges = max(vfields(2,iobs), qx_min)  ! qs
